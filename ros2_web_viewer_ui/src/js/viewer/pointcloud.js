...
function updatePointcloud2(object, msg) {
  ...
  const zThresholdMin = -0.5;
  const zThresholdMax = 2.0;

  const thresholdEnabled = document.getElementById('cbObstacle').checked;
  ...
  for (let i = 0; i < numPoints; i++) {
    ...
    if (thresholdEnabled && z >= zThresholdMin && z <= zThresholdMax) {
...
// function updateObstacle(object, msg) {
//   const bounds = msg._data_uint16.bounds;
//   const data = parsebase64(msg._data_uint16.points);
//   const view = new DataView(data);
//   const numPoints = data.byteLength / 6;
